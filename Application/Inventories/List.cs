...
namespace Application.Inventories
{
    public class List
    {
        ...
        public class Handler : IRequestHandler<Query, List<Inventory>>
        {
            ...
            {
                var portfolios = await __context.Portfolios.ToListAsync(cancellationToken);
                var securities = await __context.Securities.ToListAsync(cancellationToken);
                var holdings = await __context.Holdings.ToListAsync(cancellationToken);
